...
loop directly" guidance. Mirrors event-creator's identical Slice 1 test.
"""
...
async def test_refresh_loop_selects_the_local_dev_token_provider_when_bypass_is_true(
    ...
    try:
        await asyncio.sleep(0.05)
        # The default path's token provider is wrapped by _instrumented_token_provider and would
        # hang/fail trying to reach the (unreachable in tests) GCP metadata server - proving the
        # captured provider is the local-dev one this cheaply, by calling it, is more direct than
        # asserting on the wrapper's identity/closure internals.
