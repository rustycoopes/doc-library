...
>   itself.
>
> Example: `- 2026-07-17 — #1 Slice 1: Repo & infra setup — [details](features/doc-library/WBS/slice-1-repo-and-infra-setup.md)`
...
- 2026-07-19 — #22 Registry refresh loop's silent hang diagnosed and made observable: root cause is Cloud Run CPU throttling starving the background asyncio loop between requests (same class as organize-me's ADR-0001), not a real network/auth failure; added a bounded, explicitly-logged timeout around the OIDC token-fetch step so a redeploy can distinguish "token fetch is slow" from "the Host GET is slow" instead of producing zero log output — [issue #22](https://github.com/rustycoopes/doc-library/issues/22)
- 2026-07-28 — #11 Intake: Slice 3 non-blocking review suggestions — HTMX 422s are now visibly rendered (shared `alert` banner + `hx-on::response-error`), URL inputs enforce http(s) client-side, the JSON API's link list no longer duplicates `list_grouped_by_category`'s query, and the htmx CDN script got an SRI hash — [details](features/doc-library/WBS/slice-3-doc-link-crud.md#delivered--issue-11-follow-up)
