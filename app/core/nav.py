...
def sidebar_nav_context(host_user: HostUser | None, request: Request) -> dict[str, object]:
    ...
    Mirrors event-creator's `app.core.nav.sidebar_nav_context` - reads the user's collapsed-group
    preference from the read-only `HostUser` mapping, since Doc Library has no write path to
