...
class Settings(BaseSettings):
    ...
    # Host already exists in both environments), so this is a plain env var set directly in
    # deploy.yml/ci.yml, no post-deploy capture step needed. Empty default so local dev/CI that
    # hasn't set it yet still starts; the background refresh loop just never succeeds, and this
    # service keeps serving its self-only cold-start default (see
    # docs/adr/registry-decoupling-endpoint-auth.md in organize-me).
